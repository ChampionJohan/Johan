// ADTP 설명회 덱 생성기 — .pptx 와 동일 좌표의 HTML 프리뷰를 함께 출력한다
...
/* ══ 출력 ══════════════════════════ */
pres.writeFile({ fileName: 'ADTP_미국편입과정_안내.pptx' }).then(() => {
  const pages = html.map((h,i) => `<div class="slide ${h.dark?'dark':''}"><div class="no">${i+1}</div>${h.els.join('')}</div>`).join('');
  fs.writeFileSync('/tmp/claude-0/-home-user-Johan/eb00403c-5e26-5422-a421-25c490e75ccf/scratchpad/adtp-preview.html',
...
div{box-sizing:border-box}
  ...
  console.log('wrote ADTP_미국편입과정_안내.pptx —', html.length, 'slides');
});
