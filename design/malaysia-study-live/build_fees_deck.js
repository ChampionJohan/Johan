...
/* ── 2. 표 읽는 법 ── */
{
  ...
  const rules = [
    ...
    ['어느 통화인가','학교는 국제학생용 요금표를 달러로도 냅니다. 그러나 달러 금액은 참고용이고 실제 납부는 링깃(RM)입니다.'],
  ...
  text({ t:'학교는 국제학생용 요금표를 달러로도 발행하지만, 그 금액은 참고용입니다. 환산 기준은 고시환율이 아니라 학교 지시환율(USD 1 = RM 4.00대)이며, 환율 변동·은행 수수료 차액은 학생이 부담합니다.', x:M+0.28, y:5.6, w:5.3, h:0.66, size:11, color:C.text, lh:1.4 });
...
/* ── 4. 선웨이 학비 ── */
{
  ...
  bottom('환율 RM 1 ≈ 339원 기준 · 6% 서비스세 별도 · 경영계열은 학교가 달러로 표기한 금액을 환산한 값이므로, 학교 요금표의 링깃 원표기로 확인하십시오');
