...
/* ── 2. 표 읽는 법 ── */
{
  ...
  const rules = [
    ...
    ['어느 통화인가','요금표도 입학처 안내도 달러로 오는 경우가 있습니다. 그러나 달러는 참고용이고 실제 납부는 링깃(RM)입니다.'],
  ...
  rect({ x:M, y:5.2, w:5.86, h:1.16, fill:C.brickSoft, radius:0.07 });
  text({ t:'달러 표기를 받으셨다면', x:M+0.28, y:5.32, w:5.3, h:0.26, size:12, bold:true, color:C.brick });
  text({ t:'입학처가 달러로 안내하거나 요금표가 달러판이어도, 그 금액은 참고용입니다. 환산 기준은 고시환율이 아니라 학교 지시환율(USD 1 = RM 4.00대)이며, 환율 변동·은행 수수료 차액은 학생이 부담합니다.', x:M+0.28, y:5.6, w:5.3, h:0.66, size:11, color:C.text, lh:1.4 });
  rect({ x:M+6.07, y:5.2, w:5.86, h:1.16, fill:C.goldSoft, radius:0.07 });
  text({ t:'상담에서 쓸 한 문장', x:M+6.35, y:5.32, w:5.3, h:0.26, size:12, bold:true, color:C.gold });
  text({ t:'"학비는 3년이냐 4년이냐, 파운데이션이 붙느냐에 따라 두 배까지 벌어집니다. 아이 전공을 먼저 정하고 나서 금액을 보십시오."', x:M+6.35, y:5.6, w:5.3, h:0.66, size:12, bold:true, color:C.ink, lh:1.4 });
  bottom('※ 청구서는 링깃으로 옵니다. 이 자료의 금액을 전부 링깃 기준으로 통일한 이유입니다.');
  note('금액부터 말하면 학부모가 혼란스러워합니다. 이 네 가지를 먼저 맞추고 표로 들어가십시오. 달러 요금표를 받아 온 학부모가 있으면 왼쪽 박스를 읽어 주십시오 — 실제 납부는 링깃입니다.');
}
...
/* ── 4. 선웨이 학비 ── */
{
  ...
  bottom('환율 RM 1 ≈ 339원 기준 · 6% 서비스세 별도 · 경영계열은 입학처가 달러로 안내한 금액(연 USD 6,820~9,091)을 환산한 값입니다 — 청구 시 적용되는 학교 지시환율을 입학처에 확인하십시오');
  note('경영계열 달러 금액은 입학처에서 받은 수치입니다. 남은 변수는 청구 시 학교가 적용하는 환율 하나이므로, 그것만 확인하면 확정됩니다.');
}
...
/* ── 8. 확인 항목 ── */
{
  ...
  const items = [
    ...
    ['선웨이 경영계열 적용 환율','입학처 안내는 달러 — 청구 시 지시환율 확인'],
