...
{
  slide(false);
  head('10',"TAYLOR'S — 학비 및 글로벌 학위",'테일러스 학비와 진학 경로','국제학생 기준 · 모든 금액은 링깃(RM)과 원화로 통일했습니다');
  tableEl({ x:M, y:2.0, w:CW, rowH:0.48, size:12.5,
    colW:[2.7, 1.1, 2.6, 2.8, 2.733],
    rows:[
  ...
  const tp = [
    ['복수 학위 (Dual Award)','UWE 브리스톨(영국) · QUT(호주)\n경영 · 컴퓨팅 계열'],
    ['트위닝 · 편입','호주 디킨·모나쉬·QUT·멜버른·남호주대\n영국 샐퍼드·UWE, 뉴질랜드'],
    ['ADTP (학점 이전)','1996년 개설 · 미국 상위권 편입\n합격 보장 아님'],
  ];
  tp.forEach((r,i)=>{
    const x = M + i*4.04;
    rect({ x, y:5.0, w:3.86, h:1.24, fill:C.sand, radius:0.07 });
    text({ t:r[0], x:x+0.26, y:5.14, w:3.34, h:0.3, size:13, bold:true, color:C.deep });
    text({ t:r[1], x:x+0.26, y:5.46, w:3.34, h:0.66, size:11.5, color:C.text, lh:1.4 });
  });
  bottom('환율 RM 1 ≈ 339원 기준 · 5년은 파운데이션 1년 + 본과 4년 ·  2025년 7월부터 외국인 학비에 6% 서비스세(SST)가 별도 부과됩니다');
  note('표를 RM 기준으로 통일했습니다. 공대 지망자는 5년 행을 보여 주는 것이 정확합니다.');
}
...
{
  slide(false);
  head('12','SUNWAY — 학비 및 글로벌 학위','선웨이 학비와 진학 경로','국제학생 기준 · 달러 표기를 링깃으로 환산해 통일했습니다');
  tableEl({ x:M, y:2.0, w:CW, rowH:0.48, size:12.5,
    colW:[2.7, 1.1, 2.6, 2.8, 2.733],
    rows:[
      ['과정','기간','총 학비 (RM)','총 학비 (원화)','연간 (원화)'],
  ...
  const sp = [
    ['복수 학위 — 랭커스터','2개 학위증 · 양교 동문 자격\n추가 비용 없이 기본 등록금에 포함'],
    ['미국 파트너십 — ASU','Cintana Alliance 파트너\n20개 이상 패스웨이 · 최대 90학점 이전'],
    ['ADTP (학점 이전)','1987년 개설 · 2,000개 이상 대학 선택\n합격 보장 아님'],
  ];
  sp.forEach((r,i)=>{
    const x = M + i*4.04;
    rect({ x, y:4.52, w:3.86, h:1.24, fill:C.sand, radius:0.07 });
    text({ t:r[0], x:x+0.26, y:4.66, w:3.34, h:0.3, size:13, bold:true, color:C.deep });
    text({ t:r[1], x:x+0.26, y:4.98, w:3.34, h:0.66, size:11.5, color:C.text, lh:1.4 });
  });
  rect({ x:M, y:5.94, w:CW, h:0.6, fill:C.goldSoft, radius:0.07 });
  text({ t:'선웨이의 결정적 장점 — 랭커스터 복수 학위에 추가 비용이 없습니다. 영국에 가지 않고도 영국 학위를 받고, 등록금도 더 내지 않습니다.', x:M+0.28, y:5.94, w:CW-0.56, h:0.6, size:12.5, bold:true, color:C.ink, valign:'middle' });
  bottom('환율 RM 1 ≈ 339원 기준 · 경영계열은 입학처가 달러로 안내한 금액(연 USD 6,820~9,091)을 환산한 값 — 청구 시 적용 환율은 입학처 확인 ·  2025년 7월부터 외국인 학비에 6% 서비스세(SST) 별도 부과'); 
  note('경영계열 달러 금액은 입학처에서 받은 수치입니다. 청구 시 적용 환율만 확인하면 확정됩니다. 공대 지망자는 5년 행이 실제 예산입니다.');
}
